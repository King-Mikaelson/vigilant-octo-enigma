...
const submitForm = (values:Values) => {
  ...
  const{ setMultipleStoreState} = useContext(AuthContext );
        ...
        <section className="auth">
          ...
          <form className="form" onSubmit={(e) => handleDefault(e)}>
            <aside className="grid">
              ...
              <Button text="go back" onclick={() => {setMultipleStoreState?.("one")} } />
              {disableSecondBtn ? (
                <Button text="Create Account" onclick={() => {navigate("/admin"); setMultipleStoreState?.("one") }} />
              ) : (
...
const  SignUp2 = () => {
  const{ multipleStoreState} = useContext(AuthContext );
  return (
  <>
  {multipleStoreState === "one" ? <FormOne/> : <FormTwo/>}
  </>
  );
}
